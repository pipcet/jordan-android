...
define mv-modules
    ...
    if [ "$$mdpath" != "" ];then\
        ...
        for i in $$ko; do \
	$(ARM_EABI_TOOLCHAIN)/arm-eabi-strip --strip-unneeded $$i;\
        cp $$i $(KERNEL_MODULES_OUT)/; \
	done;\
    fi
endef
...
$(KERNEL_CONFIG): $(KERNEL_OUT)/.exists $(KERNEL_OUT)
...
TARGET_KERNEL_BINARIES: $(KERNEL_OUT)/.exists $(KERNEL_OUT) $(KERNEL_CONFIG) $(KERNEL_HEADERS_INSTALL)
...
$(KERNEL_HEADERS_INSTALL): $(KERNEL_OUT)/.exists $(KERNEL_OUT) $(KERNEL_CONFIG)
